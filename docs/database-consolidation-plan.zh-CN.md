...
**本节是当前执行记录；后文双写、Mongo 回滚和待公告描述保留为历史，不再作为操作指令。维护者已明确不需要公告。**

- 观察起点：2026-08-29 01:29（北京时间）；2026-09-05 06:50 检查时已超过 7 天。
- 生产 `read_source=postgres`、canary `read_source=mongo`，均健康且容器重启数为 0。
- 自生产当前容器启动以来，日志没有 PG shadow write 失败或 panic；8 月 31 日 23:45–23:46 有 99 条游戏数据/Redis 超时相关日志，此后未再出现同类错误。验收不应写成“七天零异常”。
- 28 组有效公开 API HTTP 200 样本，PG/Mongo 0 差异；双方 404 的样本不计入。这是有限抽样，不代表全量逐行校验。
- PG 副本 streaming，检查时 WAL 差约 0.9 MB；`/data2` 使用率 20%。

### U11 实现

PostgreSQL 成为唯一读写源，上传在持久化失败时返回错误，且不再执行后续成功通知。组卡入口也迁到 PG，保留投影限制和大整数精度。健康检查用 `game_data` 检查独立 PG pool；缺失 `GAME_DATA_URL` 在启动时拒绝。`GAME_DATA_READ_SOURCE` 仅接受 `postgres`，拒绝遗留 `mongo` 配置。

删除 Mongo 客户端、Mongo-only 剥离、迁移 CLI 与旧读翻转彩排。`mongodb.private_api_secret` / `private_api_user_agent` 及对应环境变量保留兼容；Mongo 驱动模块暂仅保留 BSON 响应兼容工具使用，不建立 Mongo 连接。

修订旧 U11 第 3 项：删除架构基线中的 `Mongo`，保留实际承载独立 PG pool 的 `GameData`；不能在消费者仍使用该能力时只删基线或把它藏到另一个聚合。

### 验证

干净源码副本（排除本机已忽略的旧 `cmd/suite-rec-backfill` 和上传样本）上，`gofmt`、`go build ./...`、`go vet ./...`、`staticcheck ./...`、`go test -race -count=1 ./...` 全部通过。全量测试包含独立 PostgreSQL 18 测试库上的写入语义、正式上传失败返回、完整字段保留、组卡投影/大整数与缓存 stamp 回归。
...
| 3 | 建表 + 部署新读写路径（`read_source` 仍为 `mongo`） | ✅ **已完成**。写路径于 2026-08-26 接入双写（§7.3.1） |
| 4 | ~~只读维护窗口开始~~ **未使用** | 双写让 PG 始终与 Mongo 同步，回填无需窗口。见 §7.1.2 |
| 5 | `gamedata-migrate migrate -apply` 全量回填 | ✅ **已完成**。放行闸改为响应层彩排，不是 `verify`——见 §7.1.2 |
| 6 | 翻转 `read_source=postgres` + **同步清空 game-data Redis 命名空间** | ✅ **已完成 2026-08-29 01:29**，窗口 6 s，清 8,325 键。Mongo 写**未停**（§7.3.1 的偏差，保留为回滚快照）|
| 7 | ~~只读窗口结束~~ **未使用** | 上传全程未中断 |
| 8 | 观察 ≥ 7 天（一个 `GameDataCacheTTL`） | ← **当前所处阶段**，自 2026-08-29 起 |
| 9 | **改为：剥离只作用于 Mongo 写入路径**，PG 存全量（§7.1.1） | ✅ **U12a 2026-08-29 00:58**、**U12b 02:33** 均已完成，见 §7.1.3。**尚欠对外公告** |
| 10 | **清理旧数据**：删除 MongoDB 的 suite / mysekai | 第 8 步观察期通过 |
| 11 | 下线 MongoDB 容器与依赖；**删除 `cmd/gamedata-migrate`**（#9） | — |

**旧数据的清理时机由维护者定为第 10 步** —— 即「确认新架构正常运行之后」，而不是切换当时。这段时间是整个方案的回滚窗口。

> 实际执行时 MongoDB **没有**停写（§7.3.1 的偏差被保留下来）。代价是 Mongo 继续增长，收益是回滚快照永远是当前的，回滚窗口没有时间边界 —— 任何时刻把读源翻回去，服务的都是最新数据而不是第 6 步那一刻的存量。第 10 步之前应该用一个开关把 Mongo 写关掉。
...
#### 7.1.2 实际切换记录（2026-08-29）

切换没有用维护窗口。§7.1 按「窗口内做全量装载」排了 45–60 分钟，但 §7.3.1 的双写让
PostgreSQL 始终与 MongoDB 同步 —— 切换当天两边计数逐位相等（`game_suite` 11,072、
`game_mysekai` 4,157），装载早已完成，窗口里没有事情可做。第 4、5、7 步因此作废。

| 动作 | 时间 | 窗口 |
|---|---|---|
| U12a：剥离下沉到 Mongo 写入路径 | 01-29 00:58 | 6 s（重建 backend） |
| 翻转 `read_source=postgres` + 清缓存 | 01-29 01:29 | **6 s** |

**上传全程未中断。**

##### 放行闸不是 `verify`

§7.1 第 5 步要求 `gamedata-migrate verify` 的 differing 为 0。**双写上线后这个条件已经
不可能满足**，原因是一个表示差异：`shamaton/msgpack/v3` 把 msgpack 的 float32 解成 Go
float32，BSON 没有 float32 只能拓宽成 double，而 `json.Marshal` 按 32 位最短表示输出 ——

```
Mongo:  0.953000009059906     (BSON 拓宽)
PG   :  0.953                 (float32 最短表示)
```

同一个值，零数据丢失，但 `verify` 会把每一篇含浮点的文档判成 differing（前 1,000 篇就报
满了 40 条上限）。§7.6 的离线演练之所以能拿到 `verify` 0，是因为那批数据由 `migrate` 从
MongoDB 装载，两边都走 float64；**双写是从 msgpack 直接写 PG 的，演练从未覆盖这条路**。

真正的闸是 §7.6 的**读翻转彩排**，它比对的是服务出的字节而不是存储的字节。切换前对着
生产两个库跑：

```
dress rehearsal: 714 responses compared across 60 suite + 39 mysekai users; 0 mismatched
```

切换后再用生产（PG）对 canary（Mongo）抽样 40 条，同样 0 不一致。

##### canary 反向成为回滚验证台

切换后把 canary 翻到 `read_source=mongo`：生产读 PG、canary 读 Mongo。任何时候都能对同
一个账号同时问两个库，而 canary 就是「回滚之后会是什么样」的实时预览。

##### 清缓存踩到的坑

`ClearNamespace` 等价于 `SCAN game_data:* + UNLINK`。在容器内用
`redis-cli -a "$REDIS_PASSWORD"` 复刻它会**死循环**：`haruki-toolbox-services-redis-1`
的环境里根本没有 `REDIS_PASSWORD`（容器内长度 0），认证失败的错误文本被当成了游标。
healthcheck 能过只是因为 compose 在解析时就把字面量插进了命令。

同一个问题在 `redis-canary` 和 CN01 的 redis 上都存在 —— 共享 compose 文件里 redis 服务
没有把 `REDIS_PASSWORD` 传进 `environment`。**从宿主机传密码**，并用 `redis-cli --scan`
（它自己处理游标）：

```bash
docker exec -e RP="$REDIS_PASSWORD" haruki-toolbox-services-redis-1 sh -c \
  'redis-cli -a "$RP" --no-auth-warning --scan --pattern "game_data:*" > /tmp/k.txt
   xargs -r -a /tmp/k.txt -n 500 redis-cli -a "$RP" --no-auth-warning UNLINK'
```

清了 8,325 键；紧接着再清一次返回 0，确认干净。

##### U12a 之后的数据积累

数据只能向前积累，已剥离的从未被存储、无法回填。割接 15 分钟后实测：新上传的 jp 行**每行
平均 1,190 kB**（剥离版约 450 kB），即每账号净增约 **0.74 MB**。

按 jp 账号的最后活跃时间分布推算填充速度：

| 最后活跃 | jp 账号数 | 预计 PG 增量 |
|---|---:|---:|
| 近 1 天 | 402 | ~300 MB |
| 近 7 天 | +320 | |
| 近 30 天 | +641（累计 1,363） | **~1.0 GB** |
| 30 天以上 | **2,948（68%）** | 大概率永不填充 |

⚠️ **这是 U12b 必须提前规划的事**：把这些键加进 `allowed_keys` 对外开放时，**约三分之二的
jp 账号仍会返回 `[]`**，而 cn／tw／kr 全是完整数据 —— 外部消费者会看到一个按区服和活跃度
分布的空洞。公告里需要说明「这些键自 2026-08-29 起记录，此前的数据不存在」。

#### 7.1.3 U12b 执行记录（2026-08-29 02:33）

两件事一次做完，窗口 17 秒。

| | 变更 | 结果 |
|---|---|---|
| A | `suite_remove_keys` 移除 `userProfileHonors` | 只剩 `userInherit` |
| B | `public_api_allowed_keys` 追加 18 个键 | **32 → 50** |

`userInherit` **永久保留**在双边剥离名单里：它是账号转移凭据，也在 `catalog.DeniedKeys`
中，两道防线都要留着。`userMaterials` 在配置里一直是注释状态，从未被剥离过。

##### 运行期配置键并不存在

§7.2 把 `allowed_keys` 描述为 Redis 支撑的运行期配置，管理端 `PUT /api/admin/config/
public-api-keys` 可改。**但 `haruki:config:runtime` 这个键在生产里根本不存在** —— 从未通过
管理端写过，所以 YAML 是唯一生效来源，`runtimeConfigSnapshotFromConfig` 的种子就是最终值。

因此这次变更走的是普通配置路径：改 YAML、重启、清缓存。不需要动 Redis，回滚就是换回备份文件
（`haruki-suite-configs.yaml.bak-u12b-20260829-022928`）。

⚠️ 但这也意味着：**一旦有人通过管理端改过一次白名单，YAML 就不再是生效来源**，之后再改
YAML 会被 Redis 里的快照盖掉。届时两边都要改。

##### 体积：gzip 把问题消解了

改前担心响应会从 4 MB 涨到 14 MB。实测同一账号：
...
游戏数据 JSON 重复度极高，压缩后不构成带宽问题。**结论：不需要为体积做任何额外设计。**

##### 验证

```
jp 382489570330185732   shopItems=44,408   statuses=122,516   missions=7,152
jp 5967095070322689     shopItems=44,408   statuses=122,516   missions=14,412
cn 7486008412963380031  shopItems=29,730   statuses=2,000     missions=1,747   honors=3
tw 7002442947828554497  shopItems=36,835   statuses=14,559    missions=995
```

##### ⚠️ 尚欠：对外公告

数据自 **2026-08-29** 起积累，**无法回填**。当前分布：

- **cn／tw／kr**：完整（§1.1.2，它们从未被真正剥离过）
- **jp／en**：只有 U12a 之后上传过的账号有。**2,948／4,311 个 jp 账号最后活跃在 30 天以前**，
  它们会长期返回 `[]`

外部消费者会看到一个按区服和活跃度分布的空洞。公告必须说明「这些键自 2026-08-29 起记录，
此前的数据从未被存储、无法补回」，否则会被当作 bug 报上来。
...
**U12b — 放开 `userProfileHonors`，并把 18 个键加进 `allowed_keys`（对外变更，可逆）**

> ✅ **已于 2026-08-29 02:33 执行**，比原计划提前 —— 原本 gate 在 U11 观察期之后，实际在
